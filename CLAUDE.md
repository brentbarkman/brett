...
- **Worktree path discipline:** When working inside a git worktree (anything under `.claude/worktrees/<name>/`), every file edit, every `cd`, and every shell command MUST use the worktree-prefixed absolute path. The bare path `/Users/brentbarkman/code/brett/<...>` resolves to the parent repo checkout, which is on a different branch — edits there silently land on the wrong branch and won't show up in the worktree's `git status`. Before editing, sanity-check with `pwd` from inside the worktree and confirm the file path begins with `.claude/worktrees/<name>/`. The same rule applies to `pnpm`, `pnpm test`, `xcodebuild`, etc. — `cd` into the worktree first, never the parent repo.
