...
- **React:** v19. **React Compiler is disabled** in the desktop build (see `apps/desktop/vite.config.ts`). v1.0.0 silently broke the packaged Electron bundle: closure hoisting detached React Router's popstate subscriber, so clicks updated the URL but the view stayed put with a clean console. Annotation mode (compile only files with `"use memo";`) was tried as a compromise and still broke navigation even with zero files annotated — the plugin is not a no-op. Until upstream ships fixes for facebook/react#35342, #36128, #34045, #35009, the compiler stays off entirely. Hand-written `useMemo` / `useCallback` / `React.memo` is fine where it measurably helps. **Do not re-enable the compiler in any mode without a successful test of the packaged build (`electron dist/electron/main.js` after `vite build`) — production shipping breakage here has cost multiple emergency releases.**
