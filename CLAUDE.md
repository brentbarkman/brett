...
- **React:** v19. React Compiler runs in **annotation mode** for the desktop build (see `apps/desktop/vite.config.ts`). Only files that start with `"use memo";` get compiled; everything else is skipped. The compiler v1.0.0 silently broke the packaged Electron bundle globally — React Router's popstate subscriber never fired re-renders, so clicks updated the URL but the view stayed put. Dev Electron was fine because the dev pipeline runs the compiler differently. Opt leaf components into `"use memo"` once you've verified they work in the **packaged** build (not just dev) — test with `electron dist/electron/main.js` after `vite build`. **Don't opt in anything that touches Router context or useSyncExternalStore-based subscriptions** until upstream compiler bugs (facebook/react#35342 et al) land fixes.  Hand-written `useMemo`/`useCallback`/`React.memo` is fine for non-annotated files.
