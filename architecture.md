...
- **React Compiler is disabled** (see top-level `CLAUDE.md` for history). Hand-rolled `useMemo` / `useCallback` / `React.memo` where it measurably helps. `useOmnibar` memoizes its return — originally because the compiler bailed on its ref patterns; the memoization is still correct and still needed since consumers depend on stable identity.
...
XcodeGen (`project.yml`), iOS 18 minimum, Swift 6 strict concurrency, 4 targets (app, share extension, tests, UI tests), ~220 Swift files in `apps/ios/Brett/`, 694 tests across 95 suites under `apps/ios/BrettTests/`.
...
- **Atomic mutations with rollback.** Every store creates the optimistic `@Model` insert AND the `MutationQueueEntry` in the same `ModelContext` save, going through a `ModelContextSaving` injection seam (`Stores/ModelContextSaving.swift`: `LiveSaver` in production, `ThrowingSaver` / `ThrowingSaverWrappingLive` in tests). `create` / `update` paths are `throws`; on save failure both the model row and the queue entry roll back together via `context.rollback()`, so the visible state and the push queue stay in lockstep. Mutations stop at the `ActiveSession.shared` boundary — `userId` is an explicit parameter and the push trigger is captured at init via the `SyncTrigger` protocol (weak, so sign-out tear-down is unblocked).
- **Mutation compaction in the write path.** Stores call `MutationCompactor.compactAndApply(_:in:)` between the optimistic SwiftData write and `saver.save()` (`Sync/MutationCompactor.swift`). Rapid same-field edits collapse — N redundant UPDATEs become 1 queue entry — keeping `/sync/push` traffic tight on flaky networks. Parity with the desktop sync engine.
- **Sync engine:** `Sync/PushEngine` (FIFO drain, batch 10, per-mutation idempotency key, field-level reconciliation via `ConflictResolver`, permanent failure threshold 10), `Sync/PullEngine` (cursor pagination with `SyncEntityMapper`), `Sync/MutationQueue`. On app launch, `in_flight` rows from a prior crash are reset. The flow is the offline-first mirror of the server's `/sync/push` + `/sync/pull`.
- **`SyncEntityMapper` is per-model `Codable`.** Each `@Model` (`Item`, `ItemList`, `CalendarEvent`, `CalendarEventNote`, `Scout`, `ScoutFinding`, `BrettMessage`, `Attachment`) declares explicit `CodingKeys` + `init(from:)` + `encode(to:)` matching the server wire format. `Sync/SyncEntityMapper.swift` is a thin dispatcher; JSON-blob fields that don't round-trip through `Codable` cleanly (`Item.contentMetadata`, `CalendarEvent.organizerJSON` / `attendeesJSON` / `attachmentsJSON` / `rawGoogleEventJSON`, `Scout.sourcesJSON`) are handled by mapper-shim dict patches outside the `Codable` boundary.
...
- **Sign-out clearing.** Every store conforms to the `Clearable` protocol (`Stores/ClearableStoreRegistry.swift`) and registers itself at init. `Session.tearDown()` fans `clearForSignOut()` out across the registry before `PersistenceController.wipeAllData()` runs, so in-memory caches and on-disk SwiftData rows are torn down in order. The protocol has no default impl — every conformer declares an explicit body so a future store with state can't silently inherit a no-op.
- **Views are `@Query`-driven with `userId` in the predicate.** Every list-bearing page (Inbox, Today, Upcoming, custom lists, Scouts roster, Calendar) splits a `*Body(userId:)` subview that takes `userId` as a stored property and constructs each `@Query` predicate inline in `init`, then is mounted with `.id(userId)` so a user switch forces a clean re-init. SwiftData's `#Predicate` macro can't capture `@Environment` values, so this init-subview pattern is the workaround. Cross-user defense is enforced at the SQLite level — predicates carry `userId == <id>` — not by post-fetch Swift filtering.
- **Navigation:** `NavStore` (`Stores/NavStore.swift`) holds one `currentDestination: NavDestination?` (sheet slot) and a `pendingPushDestinations: [NavDestination]` queue. The unified `NavDestination` enum lives in `Views/MainContainer.swift`: push-style cases (`.settings`, `.settingsTab(SettingsTab)`, `.scoutsRoster`, `.scoutDetail`, `.eventDetail`, `.listView`) feed `.navigationDestination(for:)`; sheet-style cases (`.taskDetail`, `.search`, `.feedback`, `.newScout`, `.editScout`) feed `.sheet(item:)`. `NavStore.go(to:)` auto-dispatches by `destination.isSheet`. Settings deep-links push `.settingsTab(...)` directly so the back button returns to the source in one tap.
- **Chat:** `ChatStore` is a thin coordinator (`Stores/ChatStore.swift`) over three pieces in `Stores/Chat/`: `ChatMessageBuffer` (in-memory `[itemId: [ChatMessage]]` + per-key `isStreaming` + per-key `lastError` + mutation primitives), `ChatPersister` (writes finalised `BrettMessage` rows to SwiftData), `StreamingChatClient` (URLSession-backed SSE byte stream + parser). The store wires them together and manages task lifetime through `Clearable` so a sign-out drops the live buffer cleanly.
- **Auth:** `AuthManager` is the single source of truth. Three providers: Sign in with Apple, Google (native GoogleSignIn SDK → `/api/auth/ios/google/token`), email/password. `KeychainStore` + App Group so the Share Extension can authenticate; `SharedConfig.writeCurrentUserId()` mirrors `userId` so a stale share from a prior account can't bleed into the current one.
- **AI surface:** Chat goes through the server's streaming endpoints; `SmartParser` (22+ covered edge cases) parses natural-language input from the omnibar; voice mode uses `SFSpeechRecognizer` + `AVAudioEngine`.
- **Design:** `Theme/` (`BrettColors`, `BrettTypography`, `GlassCard`, `BrettAnimations`), full VoiceOver / Dynamic Type / High Contrast / Reduce Motion support. Dark mode only. The view layer is governed by the iOS↔desktop parity rules in `CLAUDE.md` (list behavior consistency, list container chrome, Omnibar / ⌘K parity).
- **Share extension:** silent-save to an App Group queue + best-effort POST to `/share/web`; `ShareIngestor.drain()` runs on app foreground.
...
| ios | Swift Testing + XCUITest | 694 tests across 95 suites — mutation atomicity, sync engines, conflict resolver, SSE client, SmartParser, store conformance + read guards, user-scoped @Query predicates, NavStore routing, Codable round-trips per model, accessibility, E2E flow |
