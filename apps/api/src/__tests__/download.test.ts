...
vi.mock("../lib/storage-urls.js", () => ({
  getStorageUrls: () => ({
    base: "https://storage.example.com/brett",
    releasesUrl: "https://storage.example.com/brett/releases",
  ...
  getLatestVersion: vi.fn().mockResolvedValue({ version: "1.2.3", dmg: "releases/Brett-1.2.3.dmg" }),
}));
...
describe("GET /download", () => {
  ...
  it("contains a download link pointing to the DMG", async () => {
    const res = await app.request("/download");
    const html = await res.text();
    expect(html).toContain("https://storage.example.com/brett/releases/Brett-1.2.3.dmg");
